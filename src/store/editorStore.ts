...
export interface EditorState {
  ...
  landCaptions: (captions: CaptionSegment[], preset?: TextStylePreset, animation?: Clip["textAnimation"]) => void;
...
export const useEditorStore = create<EditorState>((set, get) => {
  ...
  return {
    ...
    landCaptions(captions, preset, animation) {
      const current = get().project;
      if (!current || captions.length === 0) return;
      const command = new AddCaptionsCommand(captions, current.sequence.height, preset, animation);
      get().run(command);
