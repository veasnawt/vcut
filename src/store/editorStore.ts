...
export interface EditorState {
  ...
  /** The Stickers tool's pick: imports the sticker/GIF (`api.importSticker` — an animated image asset)
   *  and places it at the playhead on a video layer ABOVE the main footage, smaller than full frame and
   *  selected, as one undo step. Resolves `true` once it's on the timeline; a failure becomes a status
   *  message. */
  addSticker: (result: StickerSearchResult) => Promise<boolean>;
  ...
  saveAsTemplate: (name: string, keepAssetIds?: string[], coverBase64?: string, tags?: string[]) => Promise<void>;
  ...
  /** How many clips are on the clipboard (0 = Paste is unavailable). */
  clipboardCount: number;
  /** Copies the selected clips; Cut also removes them. Paste puts them at the playhead, each on the track it
   *  came from, keeping their relative spacing, and selects what it created. */
  copySelectedClips: () => void;
  cutSelectedClips: () => void;
  pasteClips: () => void;
  /** Selects every clip on every track. */
  selectAllClips: () => void;
  /** Moves the playhead to the previous (-1) / next (1) clip boundary. */
  jumpToEditPoint: (direction: -1 | 1) => void;
 ...
 *  switches: copy in one project, paste in another (clips whose track doesn't exist there are skipped, and
 *  media assets referenced by id only resolve within the same project). */
...
export const useEditorStore = create<EditorState>((set, get) => {
  ...
  /** One attempt to write the CURRENT project. Everything about WHEN to save (debounce, retry, saving again
   *  after an edit that landed mid-request, flushing) lives in `SaveCoordinator`. */
  let failureToastShown = false;
  async function saveOnce(): Promise<SaveOutcome> {
    ...
    try {
      ...
      // clears `sessionExpired` — this is exactly how a retry right after signing back in
      // (`VCutApp.tsx`'s own banner) is confirmed to have actually worked. If the user switched projects
      // while this was on the wire, it says nothing about the NEW project's state, so leave that alone.
      set((state) =>
      ...
      // `sessionExpired`'s own doc comment for why this used to be indistinguishable from a plain network
      // blip and what that silently cost. Every OTHER failure keeps the transient toast (shown once per
  ...
  return {
    ...
    async addSticker(result) {
      const { projectId, project } = get();
      if (!projectId || !project) return false;
      let asset: Asset;
      try {
        asset = await api.importSticker(projectId, result);
      } catch (err) {
        get().setStatus(err instanceof Error ? err.message : String(err), "error");
        return false;
      }
      ...
      const withAsset = get().project!;
      const playhead = get().playhead;
      const duration = defaultClipDuration(asset);
      // The topmost unlocked overlay layer (any video track but the first, which holds the main
      // footage) with room for the whole sticker at the playhead — otherwise a new layer on top.
      const overlay = withAsset.sequence.tracks
        .filter((t) => t.kind === "video")
        .slice(1)
        .reverse()
        .find((t) => !t.locked && nonOverlappingStart(t, playhead, duration) === playhead);
      const commands: Command[] = [];
      let trackId = overlay?.id;
      if (!trackId) {
        const addTrack = new AddTrackCommand("video");
        commands.push(addTrack);
        trackId = addTrack.trackId;
      }
      const addClip = new AddClipCommand(trackId, asset.id, playhead);
      commands.push(addClip);
      // `scale` multiplies the automatic fit-to-frame size: a sticker starts at 40% of that, a GIF
      // (usually a wider scene) at 60%.
      commands.push(new SetClipTransformCommand(addClip.clipId, { ...IDENTITY_TRANSFORM, scale: result.type === "gifs" ? 0.6 : 0.4 }));
      get().run(new BatchCommand(result.type === "gifs" ? "Add GIF" : "Add Sticker", commands));

      const placed = Boolean(get().project && findClip(get().project!, addClip.clipId));
      if (placed) set({ selectedClipIds: [addClip.clipId] });
      return placed;
    },
    ...
    async saveAsTemplate(name, keepAssetIds, coverBase64, tags) {
      const { projectId } = get();
      if (!projectId) return;
      try {
        await api.saveAsTemplate(projectId, name, keepAssetIds, coverBase64, tags);
    ...
    copySelectedClips() {
      ...
      const entries = buildClipboardEntries(project, selectedClipIds);
      if (entries.length === 0) return;
      clipboardEntries = entries;
      set({ clipboardCount: entries.length });
      get().setStatus(entries.length === 1 ? translateText(language, "Copied clip") : translateText(language, "Copied {n} clips", { n: entries.length }));
    },
    ...
    cutSelectedClips() {
      ...
      clipboardEntries = entries;
      set({ clipboardCount: entries.length });
      get().run(new DeleteClipsCommand(selectedClipIds));
      get().setStatus(entries.length === 1 ? translateText(language, "Cut clip") : translateText(language, "Cut {n} clips", { n: entries.length }));
    },

    pasteClips() {
      const { project, playhead, language } = get();
      if (!project || clipboardEntries.length === 0) return;
      const command = new PasteClipsCommand(clipboardEntries, playhead);
      get().run(command);
      const created = command.createdClipIds.length;
      if (created === 0) {
        return get().setStatus(translateText(language, "Nothing to paste — the clips' tracks are gone or locked"), "error");
      }
      set({ selectedClipIds: command.createdClipIds });
      get().setStatus(created === 1 ? translateText(language, "Pasted clip") : translateText(language, "Pasted {n} clips", { n: created }));
    },

    selectAllClips() {
      const { project } = get();
      if (!project) return;
      set({ selectedClipIds: project.sequence.tracks.flatMap((track) => track.clips.map((clip) => clip.id)) });
    },

    jumpToEditPoint(direction) {
      const { project, playhead } = get();
      if (!project) return;
      const target = adjacentEditPoint(editPointTimes(project), playhead, direction);
      if (target !== null) get().setPlayhead(target);
    },
