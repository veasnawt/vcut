...
void main(){
  ...
  gl_FragColor = vec4(rgb * a, a);
      ...
      for (const name of ["tex", "key", "similarity", "smoothness", "despill"]) this.uniforms[name] = gl.getUniformLocation(program, name);
      return true;
    } catch {
      this.failed = true;
      return false;
  ...
  /** Keys `source` (a video or image) into a canvas of `width × height`. Returns it, or `null` if the GPU path isn't usable
   *  (then use the CPU keyer). */
  render(source: HTMLVideoElement | HTMLImageElement | HTMLCanvasElement, width: number, height: number, settings: ChromaKeySettings): HTMLCanvasElement | null {
