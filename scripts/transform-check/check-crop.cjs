...
(async () => {
  ...
  try {
    ...
    for (const width of [1440, 390]) {
      ...
      const unselectedCanvas = await page.locator('canvas').boundingBox();
      await page.locator('canvas').click({ position: { x: 20, y: 20 }, force: true });
      const cropButton = page.getByRole('button', { name: 'Crop & Rotate', exact: true }); await cropButton.waitFor();
      const selectedCanvas = await page.locator('canvas').boundingBox();
      const compactDock = await cropButton.locator('..').boundingBox();
      assert.ok(selectedCanvas.height < unselectedCanvas.height, 'transform controls should reserve space only while selected');
      assert.ok(compactDock.y >= selectedCanvas.y + selectedCanvas.height - 1, 'transform dock must sit below the canvas');
      ...
      const cropCanvas = await page.locator('canvas').boundingBox();
      const cropDock = await ruler.locator('..').boundingBox();
      assert.ok(cropDock.y >= cropCanvas.y + cropCanvas.height - 1, 'crop ruler must not cover the crop frame or handles');
      ...
      if (width >= 640) {
        ...
        await page.getByRole('button', { name: 'Zoom preview in' }).click();
        const zoomLabel = await page.getByRole('button', { name: 'Reset preview zoom' }).textContent();
        assert.ok(Number.parseInt(zoomLabel, 10) > 100, `preview zoom should exceed 100%, got ${zoomLabel}`);
      }
      const trackOptions = page.getByRole('button', { name: 'Track options' });
      const trackCount = await trackOptions.count();
      await trackOptions.last().click();
      await page.getByRole('menuitem', { name: 'Remove track' }).click();
      await page.getByRole('alertdialog').getByRole('button', { name: 'Remove track' }).click();
      assert.equal(await trackOptions.count(), trackCount - 1, 'Remove Track confirmation should remove the row');
